...
BEGIN
    ...
    SELECT CASE
               WHEN country.ID IS NULL THEN (SELECT ExternalID FROM dbo.lCountries WHERE ID = p.CitizenshipID)
               ELSE country.OfficialName END AS OfficialName,
           CASE
               WHEN country.ID IS NULL THEN (SELECT ExternalID FROM dbo.lCountries WHERE ID = p.CitizenshipID)
               ELSE country.Nation END AS Nation,
           ...
           p.DateOfBirth,
           CASE
               WHEN Sex.ID IS NULL THEN (SELECT ExternalID FROM lSex WHERE ID = p.SexID)
               ELSE Sex.Abbreviation END AS Sex,
           p.TravelDate,
           p.TransportNumber,
           CASE
               WHEN TransportType.ID IS NULL THEN (SELECT ExternalID FROM lTransportTypes WHERE ID = p.TransportTypeID)
               ELSE TransportType.Description END AS TransportType,
           p.MovementDirectionID,
           CASE
               WHEN MovementDirection.ID IS NULL THEN (SELECT ExternalID FROM lMovementDirections WHERE ID = p.MovementDirectionID)
               ELSE MovementDirection.Description END AS MovementDirection,
           p.BorderPointID,
           CASE
               WHEN BorderPoint.ID IS NULL THEN (SELECT ExternalID FROM lBorderPoints WHERE ID = p.BorderPointID)
               ELSE BorderPoint.Name END AS BorderPoint,
           BorderProvince.ProvinceID as ProvinceID,
    ...
    FROM [dbo].[PassengersStatistics] as p
             ...
             LEFT OUTER JOIN dbo.BorderPointsToProvinces as BorderProvince ON p.BorderPointID = BorderProvince.BorderPointID
    ...
    WHERE p.TravelDate >= @FromDate
      AND p.TravelDate <= @ToDate
